...
abstract class DataBoundObject {
   ...
   public function Load() {
      ...
      if (isset($this->ID)) {
        ...
        foreach($arRow as $key => $value) {
         ...
         logMessage('Funcion Load llamada ' . $strQuery, 100);
      };
   ...
   public function Save() {
      ...
      if (isset($this->ID)) {
         logMessage('Funcion Save llamada del ID ' . $this->ID, 75);
         ...
         foreach ($this->arRelationMap as $key => $value) {
            ...
            if (array_key_exists($value, $this->arModifiedRelations)) {
               ...
               } elseif (is_bool($actualVal)) {
                  $objStatement->bindValue(':' . $value, $actualVal, PDO::PARAM_BOOL);
               } else {
   ...
   public function __destruct() {
      logMessage('Funcion __destruct llamada del ID ' . $this->ID, 25);
   ...
   private function SetAccessor($strMember, $strNewValue) {
      logMessage('Funcion Set llamada del ' . $strMember . ' - ' . $strNewValue, 50);
   ...
   private function GetAccessor($strMember) {
      logMessage('Funcion Get llamada del ' . $strMember, 10);
